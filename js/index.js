// js/index.js - VERSIÓN FINAL CORREGIDA (Fix XSI funcionando)
...
document.getElementById('kmlInput').addEventListener('change', function(e) {
  ...
  reader.onload = function(event) {
    try {
      ...
      // ✅ FIX 4 CORREGIDO: Parsear con tolerancia a errores de ArcMap (xsi)
      ...
      if (parseError) {
          ...
          if (isXsiError && !kmlRaw.includes('xmlns:xsi')) {
              ...
              // ✅ CORRECCIÓN DEL BUG: Usar regex en lugar de string literal
              // El problema era que '<Document' no matcheaba '<Document id="..."'
              const fixedRaw = kmlRaw.replace(
                  /<Document(\s+)/i,  // Captura <Document seguido de espacio(s)
                  '<Document xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"$1'
              );
              
              console.log("🔧 Aplicando corrección de namespace...");
              ...
              if (!parseError) {
                  console.log("✅ KML reparado exitosamente");
              } else {
                  console.error("❌ Reparación falló. Error persistente:", parseError.textContent);
              }
          }
      }
